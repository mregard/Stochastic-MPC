function [temps,u,rulebreak]=PB(v_real,v0) 
...
% end
constraints = [constraints, G*h(1:t)<=xmax-L*xstart-V*pred_use(1:t)+slack(t)];
constraints = [constraints, -G*h(1:t)<=-xmin+L*xstart+V*pred_use(1:t)-slack(t)];
...
newconstraints = [newconstraints, pred_use==v_real(k+1:k+horizon)];
diagnostic = optimize(newconstraints,cost,sdpsettings('solver','fmincon','usex0',1));
u(k)=value(h(1));
% Simulation
z(:,k+1) = A*z(:,k)+B*u(k)+H*(v_real(k+1));
z(2,k+1) = 0;
xold=z(:,k+1);
